...
class MinFinCurator(NBUCuratorMemento):
    def __init__(self, chrono):
        self.__usd = NBUUSD(source=self.get_one_obj({'currency': 'USD'})['minfin_usd'], chrono=chrono)
        self.__eur = NBUEUR(source=self.get_one_obj({'currency': 'EUR'})['minfin_eur'], chrono=chrono)
