...
class MinFinCurator(InterBankCuratorMemento):
    def __init__(self, chrono):
        self.__usd = InterBankUSD(source=self.get_one_obj({'currency': 'USD'})['minfin_usd'], chrono=chrono)
        self.__eur = InterBankEUR(source=self.get_one_obj({'currency': 'EUR'})['minfin_eur'], chrono=chrono)
